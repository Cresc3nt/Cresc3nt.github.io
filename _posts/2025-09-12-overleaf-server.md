...
![Overleaf 插件](/images/overleaf-server/overleaf-plungin.png)

点击图中所示的加号，将部署的 Overleaf 服务器地址 `http://<your-server-ip-or-domain>:<your-port>` 填进框中。

![输入 Overleaf 服务器地址](/images/overleaf-server/input-url.png)

在左侧可以进行登录。

![登录账户](/images/overleaf-server/sign-in.png)

点击 `Login with Password`，根据提示输入账号密码。

![输入账号密码](/images/overleaf-server/input-password.png)

点击图中的按钮，便可以使用 VS Code 编辑 LaTeX 项目了。

![打开项目](/images/overleaf-server/open-project.png)

![编辑项目](/images/overleaf-server/edit-project.png)
